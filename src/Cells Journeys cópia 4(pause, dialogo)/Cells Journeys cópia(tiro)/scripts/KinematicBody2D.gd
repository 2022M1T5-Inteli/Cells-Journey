...
func move():
	...
	###condição de ao identificar que determinadas entradas estão precionadas o vetor "velocidade" assume um valor no eixo "X" multiplicado pela variavel "run"
#	if Input.is_action_pressed("ui_right"):  ###seta para direita
#		velocidade.x = 400*run 

		##reproduz animação do personagem se esta andando e no chão
#		if is_on_floor():
...
func vida():
	...
	if ScriptGlobal.life<=0 or velocidade.y > 1300:
